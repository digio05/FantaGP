...
if(isset($_SESSION['user'])) {
    ...
    $sth->bindParam(":cookie_name", $cookie_name, PDO::PARAM_STR);
    $sth->execute();
    $idUser = $sth->fetch(PDO::FETCH_ASSOC);
    // Imposta il cookie
...
<html>
    ...
    <div class="container-fluid">
              ...
              <?php
                if(isset($_SESSION['user'])) {
                    ...
                    if (!empty($_SESSION["lega"]))  {
                      $sql = "SELECT NomeSquadra FROM Squadra WHERE CodLega = :lega AND CodUtente = :user";
                      $sth = $dbh->prepare($sql);
                      $sth->bindParam(":lega", $_SESSION["lega"], PDO::PARAM_INT);
                      $sth->bindParam(":user", $idUser["Id"], PDO::PARAM_INT);
                      $sth->execute();
                      $result = $sth->fetch(PDO::FETCH_ASSOC);

                      if ($sth->rowCount() > 0) {
                        echo '<div class="username">' . $result["NomeSquadra"] . "</div>";
                      }
                    }
                    echo '
                    <div class="user-id">@'.$username.'</div><div class="lable-message">Monete<span class="number-message">';
                    echo $monete['Monete'].'</span></div></div></button>';
                }
                if(isset($_SESSION['user'])) {
                  $username = $_SESSION['user'];
                ?>
        ...
        <iframe style="margin-top: 10px;"src="https://f1-dash.com/dashboard" width="100%" height="1200px">
